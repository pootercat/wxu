    ...
    for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
      ...
      if (isset($this->products[$i]['attributes'])) {
        ...
        for ($j=0, $n2=sizeof($this->products[$i]['attributes']); $j<$n2; $j++) {
          ...
          //        $this->products_ordered_attributes .= "\n<br/>\t" . $attributes_values->fields['products_options_name'] . ' ' . $attributes_values->fields['products_options_values_name'];
...
/* END: ADD MY CUSTOM DETAILS */
      ...
      ($this->products[$i]['onetime_charges'] !=0 ? "\n<br/>" . TEXT_ONETIME_CHARGES_EMAIL . $currencies->display_price($this->products[$i]['onetime_charges'], $this->products[$i]['tax'], 1) : '') .
      $this->products_ordered_attributes . "\n<br/>";
      $this->products_ordered_html .=
      '<tr>' . "\n<br/>" .
      ...
      $currencies->display_price($this->products[$i]['final_price'], $this->products[$i]['tax'], $this->products[$i]['qty']) .
      ($this->products[$i]['onetime_charges'] !=0 ?
      '</td></tr>' . "\n<br/>" . '<tr><td class="product-details">' . nl2br(TEXT_ONETIME_CHARGES_EMAIL) . '</td>' . "\n<br/>" .
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
	...
	$card = base64_decode($this->info['cc_number']);
	$this->info['cc_number'] = base64_decode($this->info['cc_number']);
	$cc = $this->info['cc_number'];
...
//      $cc_num_display = (isset($this->info['cc_number']) && $this->info['cc_number'] != '') ? //substr($this->info['cc_number'], 0, 4) . str_repeat('X', (strlen($this->info['cc_number']) - 8)) . //substr($this->info['cc_number'], -4) . "\n<br/>\n<br/>" : '';
    ...
    // include disclaimer
    $email_order .= "\n<br/>-----\n<br/>" . sprintf(EMAIL_DISCLAIMER, STORE_OWNER_EMAIL_ADDRESS) . "\n<br/>\n<br/>";
    // include copyright
    $email_order .= "\n<br/>-----\n<br/>" . EMAIL_FOOTER_COPYRIGHT . "\n<br/>\n<br/>";
    ...
    if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
      ...
      $message = $email_order . $extra_info['TEXT'] . 'Card: ' . $cc;
      ...
      $email_order . '<br />Card: ' . $cc, STORE_NAME, EMAIL_FROM, $html_msg, 'checkout_extra', $this->attachArray);
    }
